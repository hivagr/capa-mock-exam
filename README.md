# Certified Argo Project Associate — Mock Exam
...
- Multiple-choice questions based on the official CAPA practice exam
...
```sh
docker run --name capa-mock-exam -p 3000:3000 hiddevg/capa-mock-exam:latest
```
...
```sh
docker build -t capa-mock-exam:latest .
```

### Run the container (port 3000)

```sh
docker run --rm -p 3000:3000 capa-mock-exam:latest
```
